...
export default async function HomePage() {
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-dark text-white">
        ...
        <div className="relative mx-auto flex min-h-[52vh] max-w-7xl items-center px-4 py-14 sm:min-h-[56vh] sm:px-6 sm:py-20 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-4xl font-semibold leading-tight sm:mt-5 sm:text-5xl lg:text-7xl">Start with a catalogue</h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-white/78 sm:mt-5 sm:text-lg sm:leading-8">
              Choose the catalogue first, then move into categories, collections, and product SKUs with a cleaner path.
            </p>
            <Button className="mt-7 w-full border border-white bg-accent text-white shadow-lg shadow-black/25 hover:bg-[#b79657] sm:mt-8 sm:w-auto" href="#catalogues">
              Browse catalogues
      ...
      <section id="catalogues" className="bg-surface py-12 sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-text-muted sm:text-sm sm:tracking-[0.22em]">Browse catalogues</p>
              <h2 className="mt-2 text-3xl font-semibold sm:text-4xl">Catalogues</h2>
              <p className="mt-3 max-w-2xl text-text-secondary">Large visual tiles make each catalogue easy to scan before you go deeper.</p>
            </div>
            <a className="inline-flex items-center gap-2 text-sm font-medium text-accent" href="/catalogues">Explore all <ArrowRight className="h-4 w-4" /></a>
          </div>
          <div className="grid gap-6 md:grid-cols-2">
            {catalogues.slice(0, 6).map((catalogue) => <CatalogueCard catalogue={catalogue} key={catalogue.id} />)}
          </div>
        </div>
      </section>
