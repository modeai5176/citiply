/**
 * upload-laminate-images.ts
 *
 * Bulk-uploads laminate swatch images extracted from the Laminate_*.pdf brochures.
 *
 * Expects images under `laminate-images/<collection-slug>/<SKU>.png`, where the
 * filename (minus extension) is the product's exact `sku` in the DB. These are
 * produced by the per-collection extractor (scratchpad/extract_*.py).
 *
 * For each `<SKU>.png` it:
 *   - finds the product by sku
 *   - runs the SAME pipeline as the admin uploader (webp main + thumb + blur)
 *   - uploads to S3 under `laminates/<sku>`
 ...
 *
 * Run one collection:   npm run upload:laminate-images -- <collection-slug>
 * Run all collections:  npm run upload:laminate-images
 */
...
const IMAGE_ROOT = join(ROOT, "laminate-images");
...
async function main() {
  ...
  if (!targets.length) {
    console.log(only ? `No folder laminate-images/${only}` : "No collection folders found.");
    return;
  }
  ...
  for (const collection of targets) {
    ...
    for (const file of files) {
      ...
      const { imageUrl, thumbnailUrl, blurDataUrl } = await processAndUploadImage(buffer, "laminates", sku);
